...
# Complete script to build app.html with 15 problems per module (75 total).

with open("generate_app_html.py", "r", encoding="utf-8") as f:
    base_html = f.read().split('print("Loading data definitions...")')[0]

# Generate data for 15 problems per module
...
# Add LPP 6 to 15 (Textbook classic LPPs)
lpp_titles = [
...
for idx, (title, source, tag, desc) in enumerate(lpp_titles, start=6):
  ...
  context:'{desc}',
  steps:[
    {{title:'Decision Variables',formulation:`Let <span class="var">x₁</span> = primary product activity level\\nLet <span class="var">x₂</span> = secondary product activity level`}},
...
print(f"Generated {len(lpp_probs)} LPP problems.")
