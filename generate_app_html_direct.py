...
print("Executing generate_app_html_direct.py...")
...
# Load TP problems
with open("compile_complete_perfect_hub.py", "r", encoding="utf-8") as f:
    tp_text = f.read().split("tp_raw_data = [")[1].split("print(f\"Transportation problems generated")[0]
    exec("tp_raw_data = [" + tp_text)
...
# Load ASGN, SP, MST problems
with open("compile_complete_perfect_hub.py", "r", encoding="utf-8") as f:
    asgn_text = f.read().split("asgn_problems = [")[1].split("print(f\"LPP:")[0]
    exec("asgn_problems = [" + asgn_text)
...
function toggleInfo(infoId) {
  state.hiddenInfoMap[infoId] = !state.hiddenInfoMap[infoId];
  const el = document.getElementById(infoId);
  if (el) {
    el.style.display = state.hiddenInfoMap[infoId] ? 'none' : 'block';
  }
}
...
function renderHome() {
  return `
    ...
    <p style="font-size:.86rem;color:#64748b;margin-bottom:20px;">Repository of 15 interactive step-by-step problems per module across all 5 Operations Research methods.</p>
    <div class="mod-grid">
      ${MODULES.map(m => `
...
// LPP / GENERAL STEPS RENDERER WITH (i) EXPLANATIONS
function renderGeneralStepsDetail(problem) {
  return (problem.steps || []).map((s, i) => {
    const infoId = `info-gen-${i}`;
    const isHidden = state.hiddenInfoMap[infoId];
    return `
      <div class="step-card">
        <div class="step-hd">
          <h3>
            ...
            ${s.title}
            <button class="info-btn" onclick="toggleInfo('${infoId}')" title="Toggle detailed explanation">i</button>
          </h3>
        </div>
        <div class="step-bd">
          ${s.explain ? `<div id="${infoId}" class="ppt-explain" style="display:${isHidden ? 'none' : 'block'};"><strong>ℹ️ Step Logic Explanation:</strong><br/>${s.explain}</div>` : ''}
...
// TRANSPORT RENDERER WITH USER METHOD SELECTION & (i) EXPLANATIONS
function renderTransportDetail(problem) {
  ...
  const infoId = `info-tp-${state.tpMethodIndex}-${state.tpStepIndex}`;
  const isHidden = state.hiddenInfoMap[infoId];
  ...
  return `
    <div style="margin-bottom:14px;">
      ...
      <div class="pill-row">
        ${problem.methods.map((m, i) => `
          <button onclick="setTpMethod(${i})" style="padding:7px 18px;border-radius:5px;border:1px solid;cursor:pointer;font-size:.83rem;font-weight:700;background:${state.tpMethodIndex === i ? '#2563eb' : '#fff'};color:${state.tpMethodIndex === i ? '#fff' : '#374151'};border-color:${state.tpMethodIndex === i ? '#2563eb' : '#d1d5db'};">
            ${m.name}
          </button>
        `).join('')}
      </div>
    ...
    <div class="tp-wrap">
      <table class="tp-table">
        <thead>
          <tr>
            <th style="background:#1b365d;">Source \\ Destination</th>
    ...
    <div class="step-card" style="margin-top:14px;">
      <div class="step-hd">
        <h3>
          <span class="snum">${state.tpStepIndex + 1}</span>
          ${step.title}
          <button class="info-btn" onclick="toggleInfo('${infoId}')" title="Toggle detailed explanation">i</button>
        </h3>
      </div>
      <div id="${infoId}" class="ppt-explain" style="display:${isHidden ? 'none' : 'block'};">
        <strong>ℹ️ Allocation Logic Explanation:</strong><br/>${step.explain}
      </div>
    </div>
    <div class="step-nav">
...
// ASSIGNMENT RENDERER WITH (i) EXPLANATIONS
function renderAssignmentDetail(problem) {
  const steps = problem.steps;
  const step = steps[state.asgnStepIndex];
  const infoId = `info-asgn-${state.asgnStepIndex}`;
  const isHidden = state.hiddenInfoMap[infoId];
  ...
  return `
    <div class="step-card">
      <div class="step-hd">
        <h3>
          <span class="snum">${state.asgnStepIndex + 1}</span>
          ${step.title}
          <button class="info-btn" onclick="toggleInfo('${infoId}')" title="Toggle detailed explanation">i</button>
        </h3>
      </div>
      <div id="${infoId}" class="ppt-explain" style="display:${isHidden ? 'none' : 'block'};">
        <strong>ℹ️ Hungarian Method Step Explanation:</strong><br/>${step.explain}
      </div>
    </div>
    <div class="table-wrap">
      <table class="asgn-table">
        <thead>
          <tr>
            <th style="background:#1b365d;">Resource \\ Task</th>
...
// SHORTEST PATH RENDERER WITH (i) EXPLANATIONS
function renderShortestDetail(problem) {
  const infoId = `info-sp-main`;
  const isHidden = state.hiddenInfoMap[infoId];
  ...
  return `
    <div class="step-card">
      <div class="step-hd">
        <h3>
          Shortest Path Algorithm Table
          <button class="info-btn" onclick="toggleInfo('${infoId}')" title="Toggle detailed explanation">i</button>
        </h3>
      </div>
      <div id="${infoId}" class="ppt-explain" style="display:${isHidden ? 'none' : 'block'};">
        <strong>ℹ️ Shortest Path Logic Explanation:</strong><br/>
        Iteratively determine the n-th nearest node to the origin until the destination node is reached. Record solved nodes, closest candidate node, total cumulative distance, and last connection link.
      </div>
    </div>
    <div class="table-wrap">
...
// MST RENDERER WITH (i) EXPLANATIONS
function renderMstDetail(problem) {
  const steps = problem.steps;
  const step = steps[state.mstStepIndex];
  const infoId = `info-mst-${state.mstStepIndex}`;
  const isHidden = state.hiddenInfoMap[infoId];
  ...
  return `
    <div class="step-card" style="margin:14px 0;">
      <div class="step-hd">
        <h3>
          <span class="snum">${state.mstStepIndex + 1}</span>
          ${step.title}
          <button class="info-btn" onclick="toggleInfo('${infoId}')" title="Toggle detailed explanation">i</button>
        </h3>
      </div>
      <div class="step-bd">
        <div id="${infoId}" class="ppt-explain" style="display:${isHidden ? 'none' : 'block'};">
          <strong>ℹ️ MST Logic Explanation:</strong><br/>${step.explain}
        </div>
        <div class="tp-wrap">
...
print("SUCCESSFULLY GENERATED STANDALONE PERFECT app.html!")
